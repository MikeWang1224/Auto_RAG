...
def decide_by_hard_rules(news_list: List[Tuple[str, float]], today_change: float,
                          full_texts: List[str] = None, adjusted_avg: float = None,
...
def analyze_target(db, collection, target, result_field):
    ...
    if not filtered:
        ...
        seen_text = set()
        top_news = []
        for docid, key, title, full, res, weight in sorted(
            filtered, key=lambda x: abs(x[4].score * x[5]), reverse=True
        ):
        ...
        summary = groq_analyze(news_with_scores, target, avg_score, today_price_change,
                               adjusted_avg=adjusted_avg, divergence=divergence)
